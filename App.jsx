...
const JSONBIN_BIN_ID   = "6a2c954ef5f4af5e29e97e33";   // e.g. "6657f3e7ad19ca34f88c3e5e"
const JSONBIN_API_KEY  = "$2a$10$WO62BnSlogAvNOlB2USYoer1RDTEh7zHvgPMzR4dpNaHnN2XglhJO";   // from jsonbin.io → API Keys
...
async function loadCollective() {
  try {
    ...
    // JSONBin may return the record directly, or wrapped in {record: {...}}
...
export default function App() {
  ...
  async function saveSession(){
    ...
    const newH=[saved,...history];
    setHistory(newH); save("nico_history",newH);
    setVault(true);
    // Read the latest bin first, then add to it — prevents the 30s poll from wiping the total
    try {
      const latest = await loadCollective() || {totalWh:0,reports:0,founders:1};
      const newCol = {
        totalWh: (latest.totalWh||0) + saved.totalWh,
        reports: (latest.reports||0) + 1,
        founders: Math.max(latest.founders||1, parseInt(profile?.founderNum||1)),
      };
      await saveCollective(newCol);
      setCollective(newCol);
    } catch {
      const newCol={totalWh:(collective.totalWh||0)+saved.totalWh,reports:(collective.reports||0)+1,founders:Math.max(collective.founders||1,parseInt(profile?.founderNum||1))};
      setCollective(newCol);
    }
  }
